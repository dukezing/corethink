...
class CommentController extends AdminController{
    ...
    public function index(){
        ...
        $all_menu = D('Comment')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->getAllComment($map);
        $page = new \Think\Page(D('Comment')->where($map)->count(), C('ADMIN_PAGE_ROWS'));
    ...
    public function edit($id){
        if(IS_POST){
            ...
            $this->assign('info', D('Comment')->getCommentById($id));
            $this->assign('all_model', D('Model')->getAllModel());
